...
class JwtProvider(dotenv: Dotenv) {
    ...
    private val privateKey: ECPrivateKey = KeyFactory.getInstance("EC").generatePrivate(keySpecPKCS8) as ECPrivateKey

    val accessVerifier: JWTVerifier = JWT.require(Algorithm.ECDSA256(publicKey, privateKey))
        .withIssuer(issuer)
        .withAudience(audience)
        .withSubject(ACCESS_SUBJECT)
        .build()

    private val refreshVerifier: JWTVerifier = JWT.require(Algorithm.ECDSA256(publicKey, privateKey))
        .withIssuer(issuer)
        .withAudience(audience)
    ...
    fun verify(token: String, type: TokenType) =
        runCatching {
            when (type) {
                TokenType.ACCESS -> {
