...
class UserService(
    ...
    fun create(email: String, displayName: String, password: String) = binding {
        val validEmail =
            emailValidator.validate(email).toResult().mapError { DomainError.EmailInvalid }.bind()
        ...
        val validPassword =
            passwordValidator
                ...
                .toResult()
                .mapError { error ->
                    DomainError.PasswordInvalid(error.details ?: "Improper password")
                }
                .bind()

        repository
            .getByEmail(validEmail)
            .toErrorIfNotNull { user -> DomainError.UserExists(user.email) }
            .bind()

        val hashedPassword = secretHasher.hash(validPassword)
        repository.insert(validEmail, displayName, hashedPassword).andThen { id ->
    ...
    fun refresh(token: String) =
        jwtProvider
            .verify(token, JwtProvider.TokenType.REFRESH)
            .mapError { DomainError.RefreshTokenInvalid }
            .andThen { jwt ->
                jwt.getClaim(JwtProvider.CLAIM_ID).asString().toResultOr {
    ...
    fun getIdFor(principal: JWTPrincipal): Result<String, ApplicationError> {
        return if (principal.subject == JwtProvider.ACCESS_SUBJECT) {
            val id =
                principal.getClaim(JwtProvider.CLAIM_ID, String::class)
                    ?: return Err(DomainError.AccessTokenInvalid)

            repository.getById(id).toErrorIfNull { DomainError.UserNotFound }.map { it.id }
        } else {
    ...
    fun getAsProfile(id: String) =
        repository.getById(id).toErrorIfNull { DomainError.UserNotFound }.map { it.toProfile() }
    ...
    fun requestEmailChange(id: String, newEmail: String) = binding {
        ...
        repository
            .getAll()
            .toErrorIf(
                predicate = { users -> users.any { user -> user.email == validatedNewEmail } },
                transform = { DomainError.UserExists(newEmail) }
            )
            .bind()
    ...
    fun requestVerifyEmailResend(id: String) = binding {
        val user =
            repository
                ...
                .toErrorIfNull { DomainError.UserNotFound }
                .toErrorIf(
                    predicate = { user -> user.email_verified == true },
                    transform = { DomainError.EmailVerified }
                )
                .bind()
        ...
        repository
            ...
            .toErrorIf(
                predicate = { tokens ->
                    tokens.any { token ->
                        token.user_id == user.id && token.expiry_time > Clock.System.now()
                    }
                },
                transform = { DomainError.RateLimitExceeded }
            )
            .bind()
    ...
    fun verifyEmail(verifyToken: String, email: String) = binding {
        val token =
            repository
                .getEmailVerificationToken(verifyToken)
                .toErrorIfNull { DomainError.VerifyTokenNotFound }
                .toErrorIf(
                    predicate = { token -> token.expiry_time <= Clock.System.now() },
                    transform = { DomainError.VerifyTokenExpired }
                )
                .bind()

        val user =
            repository.getById(token.user_id).toErrorIfNull { DomainError.UserNotFound }.bind()
        ...
        if (email == token.new_email) {
            ...
            .andThen { repository.updateEmail(user.id, token.new_email) }
            .andThen { repository.setEmailVerified(user.id, true) }
            .andThen { repository.invalidateEmailVerificationToken(verifyToken) }
            .bind()
    }

    fun requestPasswordResetToken(id: String, email: String, displayName: String) =
        repository
            .insertPasswordResetToken(id, randomTokenGenerator.generateSecureToken())
            .andThen { token ->
                mailSender.send(
                    email,
                    displayName,
    ...
    fun updatePassword(userId: String, currentPassword: String, newPassword: String) =
        passwordValidator
            ...
            .toErrorIf(
                predicate = { validPassword -> currentPassword == validPassword },
                transform = { DomainError.PasswordInvalid("Passwords must not match") }
            )
    ...
    fun resetPassword(resetToken: String, newPassword: String) =
        passwordValidator
            ...
            .toErrorIf(
                predicate = { token -> token.expiry_time <= Clock.System.now() },
                transform = { DomainError.ResetTokenExpired }
            )
            ...
            .andThen { repository.invalidatePasswordResetToken(resetToken) }
}
